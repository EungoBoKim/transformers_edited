...
from transformers_legal import BertConfig, BertModel
from transformers_legal.utils import logging
...
def load_tf2_weights_in_bert(model, tf_checkpoint_path, config):
    ...
    print("tf model : {}".format(len(init_vars)))
    names = []
    arrays = []
    layer_depth = []
    ...
    for full_name, array in zip(names, arrays):
        ...
        for i, m_name in enumerate(name):
            ...
            ## TODO BERT MODEL에 맞춰 POINTER 수정 요망
            #['layer_with_weights-10', '_attention_layer', '_output_dense', 'bias', '.ATTRIBUTES', 'VARIABLE_VALUE']
        ...
        if re.match(r"(\S+)\.attention\.self\.(key|value|query)\.(bias|weight)", trace) or re.match(
            ...
            #print("check reshape : {}".format(array.shape))
    ...
    print("pointer shape == array shape : {}".format(count))
...
def convert_tf2_checkpoint_to_pytorch(tf_checkpoint_path, config_path, pytorch_dump_path):
    ...
    print("word embeddings 초기값")
    print(vars(model.embeddings.word_embeddings))
